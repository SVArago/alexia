...
class RfidCardAdmin(admin.ModelAdmin):
    list_display = ('uid', 'sak', 'atqa', 'user', _user_full_name, 'is_active')
    list_filter = ('is_active', 'managed_by')
    readonly_fields = ['registered_at']
    search_fields = ('uid', 'sak', 'atqa', 'user__first_name', 'user__last_name', 'user__username')
    fields = ['uid', 'sak', 'atqa', 'user', 'managed_by', 'is_active', 'registered_at']
    raw_id_fields = ('user',)
